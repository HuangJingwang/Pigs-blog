...
let quality = 0.5
  ...
  let file = compressFile(picture, quality)
  console.log(file)
  // 若预览文件已有值，则先删除已有预览文件
 ...
 function compressFile(file,quality
  ...
  reader.onload = () => {
    ...
    img.onload = () => {
      ...
      // 获取压缩后base64格式数据
      let imgBase64_compressed = canvas.toDataURL(fileType, quality)
    ...
    // 将base 64 转换为file
      ...
      console.log(new File([u8Arr], fileName , options))
      let newFile = new File([u8Arr.buffer],fileName,options)
      // return new File([u8Arr],fileName+'.jpeg',options)
      return newFile
    }
