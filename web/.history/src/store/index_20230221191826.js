...
export default createStore({
  ...
  mutations: {
    ...
    HANDLEGROUPLIST(state, result) {
      // 去除已删除元素
      let groupData = result.data.filter((item) => {
        return item.is_delete !== 1
      })

      const findParentChildren = (items, idKey, parentKey) => {
        let parentWrapper = new Map()
        items.forEach((item) => {
          if (parentWrapper.has(item[parentKey])) {
            let children = parentWrapper.get(item[parentKey])
            children.push(item)
          } else {
            parentWrapper.set(item[parentKey], [item])
          }
        })
        return parentWrapper
      }

      console.log(findParentChildren(groupData, 'id', 'parent_id'))

      // console.log(groupData)
      // let arr = []
      // groupData.forEach((item1) => {
      //   item1.children = groupData.filter(
      //     (item2) => item2.parent_id == item1.id,
      //   )
      ...
      // groupData
      // function toTree(rootList, parent, list) {
      ...
      //   }
      //   console.log(list)
      //   for (let item2 of rootList) {
      //     item2.children = []
      //     console.log(item2)
      //     // if (rootList.length == 0) return list
      //     // toTree(rootList, item2.id, item2.children)
      //   }
      // }
      // console.log(toTree(groupData, 0, []))
